...
const ScriptExpression = require('../src/index');
const { script } = require('bitcoinjs-lib');

// Copied and adapted from https://github.com/bitcoin/bips/blob/master/bip-0341/wallet-test-vectors.json
const testVectors = [
  {
      "given": {
  ...
  },
  {
      "given": {
          ...
          "scriptTree": {
              "id": 0,
              "script": "pk(d85a959b0290bf19bb89ed43c916be835475d013da4b362117393e25a48229b8)",
              "leafVersion": 192
          }
      },
      "intermediary": {
  ...
  },
  {
      "given": {
          ...
          "scriptTree": {
              "id": 0,
              "script": "pk(b617298552a72ade070667e86ca63b8f5789a9fe8731ef91202a91c9f3459007)",
              "leafVersion": 192
          }
      },
      "intermediary": {
  ...
  },
  {
      "given": {
          ...
          "scriptTree": [
              {
                  "id": 0,
                  "script": "pk(387671353e273264c495656e27e39ba899ea8fee3bb69fb2a680e22093447d48)",
                  "leafVersion": 192
              },
              {
                  "id": 1,
  ...
  },
  {
      "given": {
          ...
          "scriptTree": [
              {
                  "id": 0,
                  "script": "pk(44b178d64c32c4a05cc4f4d1407268f764c940d20ce97abfd44db5c3592b72fd)",
                  "leafVersion": 192
              },
              {
                  "id": 1,
  ...
  },
  {
      "given": {
          ...
          "scriptTree": [
              {
                  "id": 0,
                  "script": "pk(72ea6adcf1d371dea8fba1035a09f3d24ed5a059799bae114084130ee5898e69)",
                  "leafVersion": 192
              },
              [
                  {
                      "id": 1,
                      "script": "pk(2352d137f2f3ab38d1eaa976758873377fa5ebb817372c71e2c542313d4abda8)",
                      "leafVersion": 192
                  },
                  {
                      "id": 2,
                      "script": "pk(7337c0dd4253cb86f2c43a2351aadd82cccb12a172cd120452b9bb8324f2186a)",
                      "leafVersion": 192
                  }
              ]
          ]
  ...
  },
  {
      "given": {
          ...
          "scriptTree": [
              {
                  "id": 0,
                  "script": "pk(71981521ad9fc9036687364118fb6ccd2035b96a423c59c5430e98310a11abe2)",
                  "leafVersion": 192
              },
              [
                  {
                      "id": 1,
                      "script": "pk(d5094d2dbe9b76e2c245a2b89b6006888952e2faa6a149ae318d69e520617748)",
                      "leafVersion": 192
                  },
                  {
                      "id": 2,
                      "script": "pk(c440b462ad48c7a77f94cd4532d8f2119dcebbd7c9764557e62726419b08ad4c)",
                      "leafVersion": 192
                  }
              ]
          ]
...
describe('Bitcoin output descriptor [TrScript]', function () {
  it('dadada', function () {
    const descriptor = "pk(02eb23fcd29fbc0ef1badafc973f9011d4c8f447fd9a5997a965cc01ffca267fb6)";
    const expr = ScriptExpression.parse(descriptor, 'regtest');
    assert.notOk(expr.hasRangeKey);  
    assert.strictEqual(expr.outputScripts.length, 1);
    const scriptBuf = script.fromASM('02eb23fcd29fbc0ef1badafc973f9011d4c8f447fd9a5997a965cc01ffca267fb6 OP_CHECKSIG')
    assert.deepEqual(expr.outputScripts, [scriptBuf])
    assert.deepEqual(expr.addresses, [])
  })

  function makeDescriptorTree(scriptTree) {
    if (!scriptTree) {
      return "";
    }
    if (typeof scriptTree === 'object') {
      return scriptTree.script
    }
    return `(${makeDescriptorTree(scriptTree[0])},${makeDescriptorTree(scriptTree[1])})`;
  }

  it('all BIP386 test vectors should pass', function () {
    testVectors.forEach(element => {
      const internalPubkey = element.given.internalPubkey
      const scriptTree = element.given.scriptTree

      let descriptor;
      if (scriptTree) {
        descriptor = `tr(${internalPubkey}),${makeDescriptorTree(scriptTree)})`
      } else {
        descriptor = `tr(${internalPubkey})`
      }
      
      const expr = ScriptExpression.parse(descriptor, "bitcoin");

      const expected = element.expected;
      assert.deepEqual(expr.outputScripts.map(a => a.toString('hex')), [expected.scriptPubKey]);
      assert.deepEqual(expr.addresses, [expected.bip350Address])
    });
  })
})
